...
class LabelTool():
    ...
    def get_event_dir(self):
        ...
        self.in_color_Dir = os.path.join(self.root_dir, "label_color")
        self.in_event_Dir = os.path.join(self.root_dir, "label_event")
        self.out_color_Dir = os.path.join(self.root_dir, "label_color_fill")
        self.out_event_Dir = os.path.join(self.root_dir, "label_event_fill")
    ...
    def loadDir(self):
        if self.root_dir == None:
            ...
            return
        self.label_event = os.listdir(self.in_event_Dir)
        '''for i in range(len(self.label_event)):
            try:
                with open(os.path.join(self.in_event_Dir,self.label_event[i])) as f:
                    data = f.read().split("\n")
                if data[0] != "13":
                    self.label_event.pop(i)
            except:
                break
        '''
        self.label_event.sort()
        self.label_color = os.listdir(self.in_color_Dir)
        '''  for i in range(len(self.label_color)):
            try:
                with open(os.path.join(self.in_color_Dir, self.label_color[i])) as f:
                    data = f.read().split("\n")
                if data[0] != "13":
                    self.label_color.pop(i)
            except:
                break
        '''
        self.label_color.sort()

        if len(self.label_event) == 0:
            print('No label found in the specified dir!')
            messagebox.showwarning(
                title='警告', message="对应件夹中没有标注文件")
            return
        else:
            print("num=%d" % (len(self.label_event)))

        # default to the 1st image in the collection
        self.cur_color = 1
        self.cur_event = 1
        self.total_event = len(self.label_event)
        self.total_color = len(self.label_color)
        self.loadImage()
    ...
    def loadImage(self):
        ...
        self.progLabel.config(text="%04d/%04d" % (self.cur_color, self.total_event))
        # load labels

        self.clear()
        self.imagename = self.label_color[self.cur_color - 1][:-4]
        self.eventname = self.label_event[self.cur_event - 1][:-4]
        ...
        self.in_labelfilename_color = os.path.join(self.in_color_Dir, labelname_color)
        self.in_labelfilename_event = os.path.join(self.in_event_Dir, labelname_event)
        self.out_labelfilename_color = os.path.join(self.out_color_Dir, labelname_color)
        self.out_labelfilename_event = os.path.join(self.out_event_Dir, labelname_event)
        self.show_pre_image()

    def show_pre_image(self):
        if os.path.exists(self.in_labelfilename_color):
            with open(self.in_labelfilename_color) as f:
                for (i, line) in enumerate(f):
                    if i == 0:
                        point_num = int(line.strip())
                        continue
    ...
    def save_fill(self):
        ...
        with open(self.out_labelfilename_event, 'w') as f:
            f.write('%d\n' % len(self.point_event_list))
            f.write("color_name:" + self.imagename + "\n")
    ...
    def saveImage(self):
        ...
        if self.out_labelfilename_color == '' or self.out_labelfilename_event == "":
            print("labelfilename is empty")
            return
        with open(self.in_labelfilename_color, 'w') as f:
            f.write('%d\n' % len(self.point_color_list))
            for point in self.point_color_list:
                f.write(' '.join(map(str, point)) + '\n')
        print('Image No. %d saved' % (self.cur_color))
        with open(self.in_labelfilename_event, 'w') as f:
            f.write('%d\n' % len(self.point_event_list))
            for point in self.point_event_list:
                f.write(' '.join(map(str, point)) + '\n')
        print('Image No. %d saved' % (self.cur_event))
    ...
    def prevImage(self, event=None):
        self.saveImage()
        self.save_fill()
        if self.cur_color > 1 and self.cur_event > 1:
            self.cur_color -= 1
            self.cur_event -= 1
            self.loadImage()
        else:
